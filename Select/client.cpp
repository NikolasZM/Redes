...
void handleServerResponse(int cliSocket) {
    char sizeBuffer[6] = {0};
    int n = read(cliSocket, sizeBuffer, 5);
    if (n <= 0) {
        cout << "Servidor desconectado o error de lectura" << endl;
    ...
    if (n <= 0) {
        cout << "Error leyendo tipo de mensaje" << endl;
        return;
    }
